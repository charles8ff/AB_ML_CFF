...
```{r}
prediction_LR <- predict.glm(model_glm, test_asFactor, type = "response")
absolutes <- 1*(prediction_LR>=0.5)
absolutes <- as.factor(absolutes)
metrics_GLM <- confusionMatrix(test_asFactor$blueWins, absolutes)
metrics_GLM
```
...
```{r}
prediction_BN <- predict(object = model_tree, test[-1],type = "class")
confusionMatrix(table(test$blueWins, prediction_BN))
```
