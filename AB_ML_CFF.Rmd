...
Este título se ha coronado como uno de los videjuegos con más jugadores concurrentes, con una cifra de [152 millones](https://activeplayer.io/league-of-legends/) de jugadoderes a día de hoy. Estas cifras han hecho que este juego tenga mucha actividad a su alrededor, y mucha actividad económica se haya desarrollado en su ecosistema, como merchandising no-oficial, o aplicaciones de terceros.
...
set.seed(123)
# set.seed(888) con esta semilla pasan cosas
```
...
```{r}
df_PCA <- prcomp(corr_post, scale = TRUE)
summary(df_PCA)

```
Se puede contemplar en la columna *'Cummulative Proportion'* que los 2 primeros componentes explican gran parte de los datos, y que a partir del 6 componente el porcentaje acumulado es ínfimo. Con la gráfica a continuación, gracias a la función [*fviz_eig()*](https://www.rdocumentation.org/packages/factoextra/versions/1.0.7/topics/eigenvalue) del paquete *factoextra*, se ilustra lo que acabamos de mencionar con las proporciones de varianza acumuladas.
```{r}
...
Con el gráfico a continuación se ilustra lo que aportan cada variable a las 8 dimensiones que se han elegido.
```{r}
fviz_cos2(df_PCA, choice = "var", axes = 1:8)
```
...
```
Se prosigue con una muestra de los *eigenvalues* de cada variable contra cada componente principal. Se ha invesigado al respecto y R calcula los vectores y valores con el signo "opuesto" al intuitivo, mas cambiar un vector de signo sólo cambia su orientación y no su magnitud. 
```{r, max.height='200px'}
df_PCA$x
```
...
En el apartado de [métricas](#metricas-del-modelo-de-regresión-logistica) haremos más análisis a este modelo.
...
En el apartado [métricas](#metricas-del-modelo-de-naive-bayes) lo compararemos con otros modelos.
...
En el apartado de [métricas](#metricas-del-modelo-de-árbol-de-decisión) haremos más análisis a este modelo.
...
Usamos el PCA para hacer un modelo a expensas de encontrar mejores resultados


# Modelo de Regresión logística con PCA 

```{r}
model_GLMPCA <- caret::train(
                             form = blueWins ~ ., 
                             data = train_asFactor,
...
# Métricas del Modelo de RL con PCA

```{r}
acc_GLMPCA <- model_GLMPCA$results$Accuracy
metrics_GLMPCA <- model_GLMPCA$results[,-1]
rownames(metrics_GLMPCA) <- NULL
metrics_GLMPCA
```
Es una mejora nimia respecto al anterior y su interpretabilidad es mucho más baja, puesto que perdemos las variables.

```{r}
metrics_table <- matrix(c(acc_GLMPCA, acc_GLM, acc_NB, acc_BT),ncol = 1, byrow = TRUE)
rownames(metrics_table) <- c("Regresión Logística con PCA","Regresión Logística", "Naive Bayes", "Binary Tree")
colnames(metrics_table) <- c("Exactitud (%)")
...
Se aleja de la diagonal lo suficiente dado las características del modelo.

# Conclusiones

Se pretende minimizar los falsos positivos, es decir, los casos que prediga una victoria y resulte ser una derrota-> Sensitividad?
porque jugar para acabar perdiendo o sufriendo por una partida agónica y acabar perdiendo es una sensación frustrante.
