...
En base a mi conocimiento del funcionamiento del juego, hay variables que a simple vistazo van a tener gran correlación, por cómo se estructura el *dataset*.
...
Se elimina la variable a predecir (*blueWins*).
...
Se observa una tabla con el comportamiento que se esperaba. A continuación se hace un análisis de componentes principales del *dataset* normalizado.
...
Con las 2 primeras variables se podría explicar casi todo el dataset, pero se eligen las primeras 6 para tener un alto porcentaje de explicación de la variabilidad.

Con el gráfico a continuación mostramos lo que aportan cada variable a las 6 dimensiones que hemos elegido.

```{r}
fviz_cos2(df_PCA, choice = "var", axes = 1:6)
```
...
El modelo de árbol y el modelo de Naive Nayes tienen la misma precisión(a semilla 888) y yo estoy flipando
